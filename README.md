# JavaFX 멤버쉽 관리 애플리케이션 with Oracle Database
## 개발환경 설정
### 자바와 이클립스 설치와 HelloJavaFX
<iframe width="956" height="538" src="https://youtu.be/WwiBVttXo8k" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
https://youtu.be/WwiBVttXo8k  

### SceneBuilder 설치와 환경설정 그리고 HelloJavaFXML  
https://youtu.be/udBUx-6wLto

### Oracle 설치와 애플리케이션용 DB 생성 HelloOracleDB
https://youtu.be/cnf_qzN-UEQ

## 1  
### SceneBuilder 로그인 폼
https://youtu.be/G5ypEZekaJc

### SceneBuilder 회원가입 폼과 회원정보 수정 폼
https://youtu.be/D63n9r8WRWw

### SceneBuilder 회원관리 폼
https://youtu.be/Ephh5lQ-l8k

## 2
### FXML이벤트 생성과 연결
https://youtu.be/fwtjP5wqqtw

### Oracle 데이터베이스 연결과 테이블 생성
https://youtu.be/2m0badpnQr0

## 3
### LoginController 로그인 체크 함수(관리자와 멤버)
https://youtu.be/_VifYYLQ1NA

### 로그인모듈, 관리자와 사용자 로그인 로직 만들기
https://youtu.be/ttv_vIXm2e0

### 로그인 모듈, 로그인 계정에 따른 새창 열기
https://youtu.be/cFTJUtAdWTI

## 4
### 회원가입 모듈, FXML버튼의 이벤트 정의하기
https://youtu.be/9bO-R6Q4vnQ

### 회원가입 모듈, 입력폼 체크와 SQL INSERT 실행
https://youtu.be/oLgLypQyUCM

## 5
### 회원정보수정, 초기화 인터페이스 구현과 Initializable
https://youtu.be/rZoM9W2p4s0

### 회원정보수정, 회원정보 입력값 검사와 SQL UPDATE 실행
https://youtu.be/sF33oK0_JXI

## 6
### 관리자 회원관리 테이블뷰 자료구조와 제네릭
https://youtu.be/NIO9EktdmmU

### 관리자 회원관리 테이블뷰 SQL SELECT
https://youtu.be/3xL6E_gvKAg

### 관리자 회원관리 테이블뷰 item 자료읽어서 보여주기
https://youtu.be/qGfFpGnqPZA

### 관리자 회원관리 테이블뷰 SQL DELETE
https://youtu.be/ZGBzUqWVQQw

### 관리자 회원관리 테이블뷰 SQL UPDATE
https://youtu.be/KLlBkHpqGk8
